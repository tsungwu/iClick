riskParityPortfolio <- function(data, covmat="cov",strategy="minrisk",Type="MV") {
  ...
  mySpec0 = fPortfolio::portfolioSpec()
  setEstimator(mySpec0)=COV
  suppressMessages(fPortfolio::setType(mySpec0) <- Type)

  if (strategy=="All Assets") {

    RiskParity <- FRAPO::PERC(match.fun(fPortfolio::getEstimator(mySpec0))(dataUsed0)$Sigma)
    setWeights(mySpec0)=as.numeric(FRAPO::Weights(RiskParity)/100)
    Portfolio=fPortfolio::feasiblePortfolio(dataUsed0, spec=mySpec0, constraints="LongOnly")
    Portfolio@title="All Assets Risk Parity Porfolio"


  } else {

    if (fPortfolio::getType(mySpec0)=="CVaR") {fPortfolio::setSolver(mySpec0)<- "solveRglpk.CVAR"
    } else if (fPortfolio::getType(mySpec0)=="MAD") {fPortfolio::setSolver(mySpec0) <- "solveRglpk.MAD"
    } else {fPortfolio::setSolver(mySpec0) <- "solveRquadprog"}
    ...
    ID0=which(fPortfolio::getWeights(QP0) ==0)
    ID1=which(fPortfolio::getWeights(QP0) !=0)

    RiskParity <- FRAPO::PERC(match.fun(fPortfolio::getEstimator(mySpec0))(dataUsed0[,ID1])$Sigma)
    ...
    Portfolio=fPortfolio::feasiblePortfolio(data=dataUsed0, spec=mySpec0, constraints="LongOnly")
...
riskOptimalPortfolio <- function(data, Type="AveDD",value) {
  ...
  Portfolio=fPortfolio::feasiblePortfolio(data=timeSeries::returns(dataUsed), spec=mySpec, constraints="LongOnly")
