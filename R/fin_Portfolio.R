...
.backtesting <- function(dataset, assetsRM, bench, Type,Strategy,COV,Rf=0,lambda,Constraint){
  ...
  mySpec = fPortfolio::portfolioSpec()

  suppressMessages(fPortfolio::setType(mySpec) <- Type)
  setEstimator(mySpec)=COV
  fPortfolio::setRiskFreeRate(mySpec) <- as.numeric(Rf)

  if (Constraint=="Short") {
    setSolver(mySpec)= "solveRshortExact"
  } else if (fPortfolio::getType(mySpec)=="CVaR") {
    setSolver(mySpec)= "solveRglpk.CVAR"}  else {
    setSolver(mySpec)= "solveRquadprog"
    }

  #===back testing
  myBacktestSpec = fPortfolio::portfolioBacktest()
...
Eq=paste(names(newData)[1], paste(names(newData)[-1], collapse= "+"),sep="~")
  ...
  myBacktest = fPortfolio::portfolioBacktesting(myFormula,data = newData, spec = mySpec,backtest = myBacktestSpec,constraints = Constraint, trace = FALSE)

  Weights = round(100*myBacktest$weights, 4)

  SmoothPort = fPortfolio::portfolioSmoothing(object=myBacktest, trace = FALSE)

  smoothWeights = round(100*SmoothPort$smoothWeights,2)
  dev.new();fPortfolio::backtestPlot(SmoothPort, cex = 0.6, font = 1, family = "mono", which="all")
  fPortfolio::netPerformance(SmoothPort)
...
.iClick.backTesting <- function(dat,Type,Lambda,plots="all",Rf) {
  ...
  if (Type=="MV") {
    mySpec1 = fPortfolio::portfolioSpec()
    suppressMessages(fPortfolio::setType(mySpec1) <- Type)
    fPortfolio::setRiskFreeRate(mySpec1) <- as.numeric(Rf)
    #setSolver(mySpec1)= "solveRquadprog"

    OUT.tangency=list()
    for (i in 1:length(COV)){
      setEstimator(mySpec1)=COV[i]
      myBacktest1 = fPortfolio::portfolioBacktest()
      setStrategyFun(myBacktest1)="tangencyStrategy"
      myPort1 = fPortfolio::portfolioBacktesting(myFormula,data = newData, spec = mySpec1, backtest=myBacktest1,constraints = "LongOnly",trace = FALSE)
      setSmootherLambda(myBacktest1) = Lambda
      mySmoothPort.tmp1 = fPortfolio::portfolioSmoothing(object=myPort1,trace = FALSE)
      OUT.tangency[[i]]=mySmoothPort.tmp1
    }

    OUT.GMVP=list()
    mySpec2 = fPortfolio::portfolioSpec()
    suppressMessages(fPortfolio::setType(mySpec2) <- Type)
    fPortfolio::setRiskFreeRate(mySpec2) <- as.numeric(Rf)
    for (i in 1:length(COV)){
      setEstimator(mySpec2)=COV[i]
      myBacktest2 = fPortfolio::portfolioBacktest()
      setStrategyFun(myBacktest2)=".GMVPStrategy"
      myPort2 = fPortfolio::portfolioBacktesting(myFormula,data = newData, spec = mySpec2,trace = FALSE, backtest=myBacktest2,constraints = "LongOnly")
      setSmootherLambda(myBacktest2) = Lambda
      mySmoothPort.tmp2 = fPortfolio::portfolioSmoothing(object=myPort2,trace = FALSE)
      OUT.GMVP[[i]]=mySmoothPort.tmp2
    }

  } else if (Type=="CVaR") {
    mySpec3 = fPortfolio::portfolioSpec()
    suppressMessages(fPortfolio::setType(mySpec3) <- Type)
    fPortfolio::setRiskFreeRate(mySpec3) <- as.numeric(Rf)
    setSolver(mySpec3)= "solveRglpk.CVAR"
    OUT.tangency=list()
    for (i in 1:length(COV)){
      setEstimator(mySpec3)=COV[i]
      myBacktest3 = fPortfolio::portfolioBacktest()
      setStrategyFun(myBacktest3)="tangencyStrategy"
      myPort3 = fPortfolio::portfolioBacktesting(myFormula,data = newData, spec = mySpec3,trace = FALSE, backtest=myBacktest3,constraints = "LongOnly")
      setSmootherLambda(myBacktest3) = Lambda
      mySmoothPort.tmp3 = fPortfolio::portfolioSmoothing(object=myPort3,trace = FALSE)
      OUT.tangency[[i]]=mySmoothPort.tmp3
    }

    mySpec4 = fPortfolio::portfolioSpec()
    suppressMessages(fPortfolio::setType(mySpec4) <- Type)
    fPortfolio::setRiskFreeRate(mySpec4) <- as.numeric(Rf)
    setSolver(mySpec4)= "solveRglpk.CVAR"
    OUT.GMVP=list()
    for (i in 1:length(COV)){
      setEstimator(mySpec4)=COV[i]
      myBacktest4 = fPortfolio::portfolioBacktest()
      setStrategyFun(myBacktest4)=".GMVPStrategy"
      myPort4 = fPortfolio::portfolioBacktesting(myFormula,data = newData, spec = mySpec4, trace = FALSE, backtest=myBacktest4,constraints = "LongOnly")
      setSmootherLambda(myBacktest4) = Lambda
      mySmoothPort.tmp4 = fPortfolio::portfolioSmoothing(object=myPort4,trace = FALSE)
  ...
  dataRefreshCode <- function(...)  {
    ...
    if (type == 2) {
      if (plots=="no"){} else if (plots=="simple") {
        ...
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[1]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[1]], cex = 0.6, font = 1, family = "mono", which="all")}
      fPortfolio::netPerformance(OUT.tangency[[1]])
    ...
    if (type == 3) {
      ...
      if (plots=="simple") {
        par(mfrow=c(3,1))
       dev.new; fPortfolio::backtestPlot(OUT.tangency[[2]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new; fPortfolio::backtestPlot(OUT.tangency[[2]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[2]])
    ...
    if (type == 4) {
      ...
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[3]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[3]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[3]])
    ...
    if (type == 5) {
       if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[4]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[4]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[4]])
    ...
    if (type == 6) {
       if (plots=="simple") {
        par(mfrow=c(3,1))
         dev.new;fPortfolio::backtestPlot(OUT.tangency[[5]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[5]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[5]])
    ...
    if (type == 7) {
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[6]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[6]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[6]])
    ...
    if (type == 8) {
        if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[7]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.tangency[[7]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.tangency[[7]])
    ...
    if (type == 11) {
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[1]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[1]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[1]])
    ...
    if (type == 12) {
        if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[2]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[2]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[2]])
    ...
    if (type == 13) {
      ...
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[3]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[3]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[3]])
    ...
    if (type == 14) {
      ...
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[4]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[4]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[4]])
    ...
    if (type == 15) {
      ...
      if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[5]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[5]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[5]])
    ...
    if (type == 16) {
        ...
        if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[6]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[6]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[6]])
    ...
    if (type == 17) {
        ...
        if (plots=="simple") {
        par(mfrow=c(3,1))
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[7]], cex = 0.6, font = 1, family = "mono", which=c(4:6))
        par(mfrow=c(1,1))
      } else if (plots=="all"){
        dev.new;fPortfolio::backtestPlot(OUT.GMVP[[7]], cex = 0.6, font = 1, family = "mono", which="all")} else if (plots=="no"){}
      fPortfolio::netPerformance(OUT.GMVP[[7]])
...
.GMVPStrategy <- function(data, spec = fPortfolio::portfolioSpec(), constraints = "LongOnly", backtest = fPortfolio::portfolioBacktest()) {
  strategyPortfolio <- try(fPortfolio::minriskPortfolio(data, spec, constraints))
  if (inherits(strategyPortfolio) == "try-error") {
    strategyPortfolio <- fPortfolio::minvariancePortfolio(data,spec,constraints)
  }
  strategyPortfolio
}
